...
Viyi.Strings 是从 [Viyi.Util][viyi_util] 中分离出来的，专注于处理字符串相关内容的一个库。

> [!note]
> 
> **English [en]**
>
> Viyi.Strings is a library separated from [Viyi.Util][viyi_util], which focuses on processing strings. 
...
>
> - [Viyi.Util](https://gitee.com/jamesfancy/Viyi.Util) 2 提供工具扩展函数 (Viyi.Util is a toolkit contians lots of extension methods)
> - [Viyi.Strings](https://gitee.com/jamesfancy/Viyi.Strings) 用于处理字符串 (Viyi.Strings is a string toolkit.)
> - [Viyi.Bytes](https://gitee.com/jamesfancy/Viyi.Bytes) 用于处理二进制数据，计划中 (Viyi.Bytes is a binary data toolkit, in planning.)

> [!tip]
>
> Viyi.Strings 在 1.4.0 开始增加 .NET 10.0，同时放弃 .NET 6.0（Microsoft 已经不再支持 .NET 6.0）。
...
- [基于文本的编码/解码](https://gitee.com/jamesfancy/Viyi.Strings/wikis/%E6%96%87%E6%9C%AC%E7%BC%96%E7%A0%81%E5%92%8C%E8%A7%A3%E7%A0%81%20(Viyi.Strings.Codec))
  
    > 基于文本的编码/解码主要依据 [RFC-4648 (rfc-editor.org)][rfc_4648]。
    
    - Base 64 编码/解码：`EncodeBase64()`/`DecodeBase64()`
        - Base 64 URL 编码/解码：`EncodeBase64Url()` / `DecodeBase64Url()`
        - 兼容 Base 64 和 Base 64 URL 的解码：`DecodeBase64Compatible()`
    
    - Base 16（十六进制）编码/解码：`EncodeBase16()`/`DecodeBase16()`
        - 别名：`EncodeHex()`/`DecodeHex()`

	- Base 32 编码/解码：`EncodeBase32()`/`DecodeBase32()`
        - Base 32 Hex 编码/解码：`EncodeBase32Hex()` / `DecodeBase32Hex()`
    
    - UTF8 编码/解码：`EncodeUtf8()`/`DecodeUtf8()`
    
        > 使用 `System.Text.Encoding.UTF8` 实现
    
    - 其他 Encoding 编码/解码快捷调用扩展方法：`Encode()`/`Decode()`
    
    - `TextCodec` 管理各心及抽象化接口
    
- [空字符串和空白字符串](https://gitee.com/jamesfancy/Viyi.Strings/wikis/%E7%A9%BA%E5%AD%97%E7%AC%A6%E4%B8%B2%E5%92%8C%E7%A9%BA%E7%99%BD%E5%AD%97%E7%AC%A6%E4%B8%B2)
  
    - `IsEmpty()` 和 `IsSpaces()` 灵活判断
        - `IsNotEmpty()` 和 `IsNotSpaces()`
    - `EmptyAs()` 和 `SpacesAs()` 灵活赋予默认值
    - `EmptyAsNull()` 和 `SpacesAsNull()`
    
- [命名风格转换（大小规则写转换）](https://gitee.com/jamesfancy/Viyi.Strings/wikis/%E5%91%BD%E5%90%8D%E9%A3%8E%E6%A0%BC%E8%BD%AC%E6%8D%A2%20CaseConvert)
    - `CamelCase()`/`PascalCase()`/`KebabCase()`/`SnakeCase()` 扩展方法
    - `CaseTo(string)` 扩展方法按自定义规则转换
    - `CaseConvert` 注册中心和 `ICaseConverter` 接口
    
- [在整数和字符串之间进行 2~36 进制转换](https://gitee.com/jamesfancy/Viyi.Strings/wikis/%E6%95%B4%E6%95%B0%E7%9A%84%E8%BF%9B%E5%88%B6%E8%BD%AC%E6%8D%A2)
    - `ToString(int radix)`
...
- [解析为布尔值](https://gitee.com/jamesfancy/Viyi.Strings/wikis/%E8%A7%A3%E6%9E%90%E4%B8%BA%E5%B8%83%E5%B0%94%E7%B1%BB%E5%9E%8B%20(bool))，灵活支持 `on/off`、`yes/no` 及其他字符串内容
    - `ToBoolean()` 系列扩展方法
    - `Booleans.Truthy` 和 `Booleans.Falsy` 提供常用真/假值集合
...
- 数字范围生成/解析
    ...
    - `NumSeries.Parse(string)` / `new NumSeries().Parse(string)`，从范围表达的字符串解析出序号列表。
    - `NumSeries.ToSeriesString(IEnumerable<int>)`，

      > 注意：`ToSeriesString()` 不会对数据源进行排序，若有必要请先调用 `OrderBy()` 扩展方法排序。
...
- 解析表示时间长度的字符串，如 `5m`、`2hours` 等

    - `string.ToTimeSpan(string defaultUnit = "m")`
        - 可以解析分钟 (`m/minutes`)、小时 (`h/hours`)、秒 (`s/seconds`)、天 (`d/days`)、周 (`w/weeks`) 及毫秒 (`ms/milliseconds`) 等
        - 数值部分目前仅支持整数

- 扩展 `IEnumerable<T>.JoinString()` 方法，封装 `string.Join()` 使之方便用于链式调用。

- [对 `FormattableString` 预先进行一次格式化处理](https://gitee.com/jamesfancy/Viyi.Strings/issues/I5HNMY)

    在使用 `ExecuteSqlInterpolated()` 前可以使用 `PreFormat` 预先处理掉不是 SQL 参数的插值（比如表名）。部分新 EF 库已经能够正常处理表名插值，但这个预处理方法还是可以用在其他方面。
